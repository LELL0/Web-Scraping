...
def scrape(soup):
    try:
        ...
        writeToLogs("ERROR while getting tags from soup")
    ...
    try:
        ...
        writeToLogs("error splitting tags")

    if not socials:
        writeToLogs("! No links were Found")
        return 0

    saveFile(socials, "AllFoundUrls")
    ...
    emptyDict = True
    for elm in cleanedDict:
        if cleanedDict[elm] != "N/A":
            emptyDict = False

    if emptyDict:
        saveFile(cleanedDict, "CleanOutput")
        return 0
    else:
        saveFile(cleanedDict, "CleanOutput")
...
def cleanurl(url):
    try:
        ...
        writeToLogs("ERROR cleaning the URL")
    return url
...
def checkIfIp(url):
    try:
        ...
        writeToLogs("ERROR checking if its an IP")
    return result
...
displayResultFromJson("CleanOutput")
